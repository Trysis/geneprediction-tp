...
def has_shine_dalgarno(shine_regex: Pattern, sequence: str, start: int, max_shine_dalgarno_distance: int) -> bool:
    ...
    for match in shine_regex.finditer(sequence, max(start - max_shine_dalgarno_distance, 0), start - 5):
        if match is not None:
            is_found = True
...
def predict_genes(sequence: str, start_regex: Pattern, stop_regex: Pattern, shine_regex: Pattern, 
    ...
    actual_position = 0
    gene_list = []
    while (len(sequence) - actual_position) >= min_gap:
        actual_position = find_start(start_regex, sequence, actual_position, len(sequence))
        if actual_position is not None:
            stop = find_stop(stop_regex, sequence, actual_position)
            if stop is not None:
                correct_len = stop - actual_position + 3 >= min_gene_len
                if correct_len:
                    has_shine = has_shine_dalgarno(shine_regex, sequence, actual_position, max_shine_dalgarno_distance)
                    if has_shine:
                            gene_list.append([actual_position + 1, stop + 3])
                            actual_position = stop + 3 + min_gap
                    else:
...
def main() -> None: # pragma: no cover
    ...
    genes = sorted(predict_genes(sequence, start_regex, stop_regex, shine_regex, 
    ...
    # Write gene
    write_genes(args.fasta_file, sequence, genes, sequence_rc, genes_rv)
